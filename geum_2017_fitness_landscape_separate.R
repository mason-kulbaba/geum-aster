...
###########################################################################
#again for comparison, try separate analyses for each Region

#split redata2017 into region-specific data
...
aouta<- aster(resp~varb, pred, fam, varb, id, root, data=redata.P,famlist = famlist)

summary(aouta, show.graph=T, info.tol=1e-13)

aout3<- aster(resp~varb + Dist.from.cg.km + No.Days.to.Germ + I(Dist.from.cg.km^2) + I(No.Days.to.Germ^2) + I(2*Dist.from.cg.km*No.Days.to.Germ), pred, fam, varb, id, root, 
              data=redata.P, famlist = famlist)
...
pout <- predict(aout)
pout <- matrix(pout, nrow = nrow(aouta$x), ncol = ncol(aouta$x))
colnames(pout) <- colnames(aouta$x)
mufit <- pout[, grep("sm2017", colnames(pout))]
...
plot(dat2.P$Dist.from.cg.km, dat2.P$No.Days.to.Germ, xlab = "Dist.", 
     ylab = "Days to Germ", col=dat2.P$Region, pch=16)
...
aouta<- aster(resp~varb, pred, fam, varb, id, root, data=redata.GLA,famlist = famlist)

summary(aouta, show.graph=T, info.tol=1e-13)

aout3<- aster(resp~varb + Dist.from.cg.km + No.Days.to.Germ + I(Dist.from.cg.km^2) + I(No.Days.to.Germ^2) + I(2*Dist.from.cg.km*No.Days.to.Germ), pred, fam, varb, id, root, 
              data=redata.GLA, famlist = famlist)
...
plot(dat2.GLA$Dist.from.cg.km, dat2.GLA$No.Days.to.Germ, xlab = "Dist.", 
     ylab = "Days to Germ", col=dat2.GLA$Region, pch=16)
...
aouta<- aster(resp~varb, pred, fam, varb, id, root, data=redata.MBA,famlist = famlist)

summary(aouta, show.graph=T, info.tol=1e-13)

aout3<- aster(resp~varb + Dist.from.cg.km + No.Days.to.Germ + I(Dist.from.cg.km^2) + I(No.Days.to.Germ^2) + I(2*Dist.from.cg.km*No.Days.to.Germ), pred, fam, varb, id, root, 
              data=redata.MBA, famlist = famlist)
...
#perform linear analysis
wmout <- lm(wmu ~ dat2.MBA$Dist.from.cg.km)
...
plot(dat2.MBA$Dist.from.cg.km, dat2.MBA$No.Days.to.Germ, xlab = "Dist.", 
     ylab = "Days to Germ", col=dat2.MBA$Region, pch=16)
